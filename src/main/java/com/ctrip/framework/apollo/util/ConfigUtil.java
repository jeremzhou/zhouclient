...
import com.ctrip.framework.apollo.enums.ResourceType;
...
public class ConfigUtil {
  ...
  private int loadConfigQPS = 1; //2 times per second
  private int longPollQPS = 1; //2 times per second
  //for on error retry
  ...
  public Env getApolloEnv() {
    ...
    if (env == null) {
      String path = isOSWindows() ? "application.yml" :
  ...
  public String getLocalIp() {
    ...
    Object clientIpObj = System.getProperties().get(ConfigConstants.UAPOLLO_CLIENT_IP);
    if (clientIpObj != null) {
  ...
  public String getDefaultLocalCacheDir() {
      ...
      return System.getProperty("spring.property.path");
  }
  ...
  private final void initInputProperties() {
      
      logger.info("initInputProperties begin to run.");
      final Properties sysProperties =  System.getProperties();
      if (!sysProperties.containsKey(ConfigConstants.APP_ID) && sysProperties.containsKey(ConfigConstants.SPRING_APPLICATION_NAME)) {
          System.getProperties().put(ConfigConstants.APP_ID, sysProperties.get(ConfigConstants.SPRING_APPLICATION_NAME));
      }
      if (!sysProperties.containsKey(ConfigConstants.ENV) && sysProperties.containsKey(ConfigConstants.UAPOLLO_ENV)) {
          System.getProperties().put(ConfigConstants.ENV, sysProperties.get(ConfigConstants.UAPOLLO_ENV));
      }
      if (!sysProperties.containsKey(ConfigConstants.DEV_META) && sysProperties.containsKey(ConfigConstants.UAPOLLO_SEVER_PORT)) {
          System.getProperties().put(ConfigConstants.DEV_META, "http://" + sysProperties.get(ConfigConstants.UAPOLLO_SEVER_PORT));
      }
      if (!sysProperties.containsKey(ConfigConstants.UAPOLLO_CLIENT_IP) && sysProperties.containsKey(ConfigConstants.UAPOLLO_CLIENT_IP)) {
          System.getProperties().put(ConfigConstants.UAPOLLO_CLIENT_IP, sysProperties.get(ConfigConstants.UAPOLLO_CLIENT_IP));
      }
      logger.info("initInputProperties end to run. app.id: {} env: {} dev_meta: {} uapollo.client.ip: {}",
              sysProperties.get(ConfigConstants.APP_ID),
              sysProperties.get(ConfigConstants.ENV),
              sysProperties.get(ConfigConstants.DEV_META),
              sysProperties.get(ConfigConstants.UAPOLLO_CLIENT_IP));
  }
  
  private final void initYamlConfig() {
      
      logger.info("initYamlConfig begin to run.");
      Properties properties = ResourceUtils.yaml2Properties(ResourceType.RESOURCE, "application.yml");
      logger.debug("initYamlConfig application.yml to properties: {}", properties);
      if (properties != null) {
          mergeConfigValue(properties);
      }
      
      String propertyPath = System.getProperties().getProperty("spring.property.path");
      if (propertyPath == null) {
          return;
      }
      String configYmlFile = System.getProperties().getProperty("spring.config.location");
      if (configYmlFile != null) {          
          
          configYmlFile = configYmlFile.replace("file:${spring.property.path}", propertyPath);
          logger.info("initYamlConfig cofnigYmlFile: {}", configYmlFile);
          properties = ResourceUtils.yaml2Properties(ResourceType.CONFIG, configYmlFile);
          logger.debug("initYamlConfig cofnigYmlFile: {} to properties: {}", configYmlFile, properties);
          if (properties != null) {
              mergeConfigValue(properties);
          }
      }

      logger.info("initYamlConfig end to run. app.id: {} env: {} dev_meta: {} uapollo.client.ip: {}",
              System.getProperties().get(ConfigConstants.APP_ID),
              System.getProperties().get(ConfigConstants.ENV),
              System.getProperties().get(ConfigConstants.DEV_META),
              System.getProperties().get(ConfigConstants.UAPOLLO_CLIENT_IP));
  }
  
  private final void mergeConfigValue(Properties properties) {
      
      final Properties sysProperties =  System.getProperties();
      if (!sysProperties.containsKey(ConfigConstants.APP_ID) && properties.containsKey(ConfigConstants.SPRING_APPLICATION_NAME)) {
          System.getProperties().put(ConfigConstants.APP_ID, properties.get(ConfigConstants.SPRING_APPLICATION_NAME));
      }
      if (!sysProperties.containsKey(ConfigConstants.ENV) && properties.containsKey(ConfigConstants.UAPOLLO_ENV)) {
          System.getProperties().put(ConfigConstants.ENV, properties.get(ConfigConstants.UAPOLLO_ENV));
      }
      if (!sysProperties.containsKey(ConfigConstants.DEV_META) && properties.containsKey(ConfigConstants.UAPOLLO_SEVER_PORT)) {
          System.getProperties().put(ConfigConstants.DEV_META, "http://" + properties.get(ConfigConstants.UAPOLLO_SEVER_PORT));
      }
      if (!sysProperties.containsKey(ConfigConstants.UAPOLLO_CLIENT_IP) && properties.containsKey(ConfigConstants.UAPOLLO_CLIENT_IP)) {
          System.getProperties().put(ConfigConstants.UAPOLLO_CLIENT_IP, properties.get(ConfigConstants.UAPOLLO_CLIENT_IP));
      }
  }
}
